...
map  ]r   :call Python_Run()<CR>
vmap ]r   :call Python_Run()<CR>
...
nmenu <silent> &Python.Next\ Line\ wrt\ indent<Tab>]<down> 
    \]<down>
"nmenu &Python.-Sep7- :
"vmenu <silent> &Python.Run\ Selection<Tab>]# 
"    \]#
"nmenu <silent> &Python.Run\ Selection<Tab>]# 
"    \]#
...
function! Python_Run ()
  "
  if &filetype != "python"
    echohl WarningMsg | echo expand("%").' seems not to be a python file' | echohl None
    return
  endif
  ...
  if g:Python_OutputGvim == "xterm"
    ...
    if  s:MSWIN
      ...
      silent exe '!xterm -title '.l:fullname.' '.s:Python_XtermDefaults.' -e '.s:python_Wrapper.' python '.l:switches.l:fullname.l:arguments
			:redraw!
    endif
    "
  endif
